...
def run_stage2_for_station(st_cfg, global_cfg, run_cfg, reuse_split=None):
    ...
    for tid in range(int(st_cfg["turbine_start"]), int(st_cfg["turbine_end"])+1):
        ...
        mdl_p1, train_mode1 = fit_mlp_center(
            ...
            thresholds_cfg=th, prated_used=estimate_prated_from_series(S.loc[idx_train,"power"]))
        sw.lap("fit center (Pass1)")

        # --- 全量（模型空间）与（清洗空间） ---
        Sv_s_m, Sr_s_m = scaler.transform(Xv_S_raw, Xr_S_raw_model if rho_for_model else None, rho_for_model)
        pred_p1 = predict_mlp_center(mdl_p1, Sv_s_m, (Sr_s_m if rho_for_model else None),
        ...
        if cleaning_passes == 0:
            ...
            _ensure_bool_flags(df_out, ["Pass1_异常", "Pass2_异常"])
            df_out.loc[S.index, "Pass1_异常"] = False
            df_out.loc[S.index, "Pass2_异常"] = False
        ...
        # === z 空间 predict 函数（有限差分回退） ===
        def predict_fn_z(Z_batch):
            Z_batch = np.asarray(Z_batch, float)
            d_clean = Z_batch.shape[1]
            d_model = 2 if rho_for_model else 1
            if d_clean != d_model:
                raise RuntimeError("predict_fn_z called with mismatched dim; rho_for_clean != rho_for_model")
            zv = Z_batch[:, 0].reshape(-1)
            zr = (Z_batch[:, 1].reshape(-1) if rho_for_model else None)
            y = predict_mlp_center(mdl_p1, zv, zr, pr_used, use_rho=rho_for_model, clip_to_prated=True)
            return y.reshape(-1)

        # === autograd 版 torch 前向（供 KNN 求梯度，优先使用） ===
        def predict_torch_z(Zb_t: torch.Tensor) -> torch.Tensor:
            """
            Zb_t: (B,d_clean) torch.Tensor (与 mdl_p1 同设备)
            返回: (B,) torch.Tensor，可微（autograd 用）
            """
            model = mdl_p1
        ...
        # === KNN Pass1 ===
        # 维度一致则使用 grad_mode 配置，否则强制 physics
        grad_mode_for_pass1 = (GRAD_MODE if (((rho_for_clean and rho_for_model) or ((not rho_for_clean) and (not rho_for_model))))
                               else "physics")
        ...
        outs1 = method.compute(
            ...
            cfg={
                **th,
                ...
                "minmax": {"a": a, "b": b},
                "predict_fn": predict_fn_for_pass1,       # 有 torch 版会优先用 torch
                "predict_torch": predict_torch_for_pass1, # autograd 优先
                "residuals": res_p1,
        ...
        def predict_fn_z2(Z_batch):
            Z_batch = np.asarray(Z_batch, float)
            d_clean = Z_batch.shape[1]
            d_model = 2 if rho_for_model else 1
            if d_clean != d_model:
                raise RuntimeError("predict_fn_z2 called with mismatched dim; rho_for_clean != rho_for_model")
            zv = Z_batch[:, 0].reshape(-1)
            zr = (Z_batch[:, 1].reshape(-1) if rho_for_model else None)
            y = predict_mlp_center(mdl_center, zv, zr, pr_used, use_rho=rho_for_model, clip_to_prated=True)
            return y.reshape(-1)

        def predict_torch_z2(Zb_t: torch.Tensor) -> torch.Tensor:
            model = mdl_center
            dev = next(model.parameters()).device
            X = Zb_t[:, : (2 if rho_for_model else 1)].to(dev).float()
            model.eval()
        ...
        grad_mode_for_pass2 = (GRAD_MODE if (((rho_for_clean and rho_for_model) or ((not rho_for_clean) and (not rho_for_model))))
                               else "physics")
        ...
        _ensure_bool_flags(df, ["Pass1_异常", "Pass2_异常"])
        df.loc[S.index, "Pass1_异常"] = np.asarray(is_p1, dtype=bool)
        df.loc[S.index, "Pass2_异常"] = np.asarray(is_p2, dtype=bool)
