...
9. [代码使用指引](#9-代码使用指引)  
   ...
   - 9.2 生成对比报告  
   - 9.3 关键配置参数说明  
   - 9.4 分析实验结果（analyze_stage2_by_turbine.py）  
...
#### 根本原因：`cleaning_passes == 0` 分支中的 `return {}` 提前终止了整个函数

当 `cleaning_passes == 0`（即 A_rules_only 组，仅规则清洗，不运行 MLP+KNN）时，`run_stage2_for_station()` 中对应分支的末尾原本写的是：

```python
if cleaning_passes == 0:
    ...
    df_out.to_csv(out_csv, index=False, encoding="utf-8-sig")
    return {}  # ← Bug：直接退出了整个函数，而非跳到下一台风机
```

`return {}` 执行的是**退出整个 `run_stage2_for_station()` 函数**，而不是跳过当前风机、处理下一台。因此，每个站点只有 `for tid in turbine_ids` 循环中**第一个**被处理的风机（即每个站点排序后的第一台：1号机、11号机、21号机、31号机、41号机、51号机）能生成输出文件，其余风机因为函数已退出而永远不被执行。

B–J 组的 `cleaning_passes > 0`，从不进入这个分支，因此正常处理了所有风机。

#### 修复方案：将 `return {}` 改为 `continue`

`stage2_modular/pipeline/orchestrator.py` 中相关代码已修复：
...
if cleaning_passes == 0:
    ...
    df_out.to_csv(out_csv, index=False, encoding="utf-8-sig")
    return {}   # 提前退出函数，后续所有风机被跳过

# 修复后
if cleaning_passes == 0:
    ...
    df_out.to_csv(out_csv, index=False, encoding="utf-8-sig")
    continue    # 跳到 for 循环的下一台风机，处理完所有风机后正常 return splits_saved
```

`continue` 将控制权交还给 `for tid in turbine_ids` 循环，所有风机依次处理完毕后，函数正常到达末尾的 `return splits_saved`。这同时也修复了 B–J 组的 `reuse_split_from: "A_rules_only"` 功能——原先 `return {}` 返回空字典，导致 A 组的划分索引未被缓存，现在能正确返回所有风机的划分供后续组复用。
...
`main.py` 完成清洗流程后，用 `analyze_stage2_by_turbine.py` 对各 run 的输出 CSV 进行逐风机统计和 MLP 对比分析。
...
| `--pattern` | `**/*_stage2_mlp.csv` | 匹配各 run 目录下输出文件的 glob 模式 |
...
| `--fast_epochs` | 不限 | 临时限制 MLP 训练轮数（调试用） |
| `--no_split_repo` | 否 | 禁用 split_repo，改为读 CSV 中的 `split` 列 |
| `--debug` | 否 | 打印详细调试信息 |

#### 输出文件

- **`turbine_results.csv`**：每行一台风机 × 一个 run，包含异常率统计和 MLP 验证集 MSE。  
- **`run_summary.csv`**：按 `n_scope` 加权汇总各 run 的指标，用于横向对比实验组。
...
*本报告 v2.2 在 v2.1 基础上新增了 Q6 问答，并在 `orchestrator.py` 中实现了 turbine_auto_discover 功能，使配置文件中无需手动维护风机编号范围。*

*本报告 v2.3 修正了 Q6 的根因分析：实际 Bug 是 `cleaning_passes == 0` 分支末尾的 `return {}` 提前退出了整个函数（而非 turbine_start/turbine_end 配置范围不完整）；`orchestrator.py` 中已将 `return {}` 改为 `continue`，确保 A_rules_only 组处理所有风机并正确缓存划分索引。*

*本报告 v2.4 新增 §9.4，说明 `analyze_stage2_by_turbine.py` 的完整命令行用法；同步新增根目录 `README.md` 作为快速上手指引。*
